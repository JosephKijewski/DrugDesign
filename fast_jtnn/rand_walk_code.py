...
class WalkDataset(Dataset):
    def __init__(self, smiles_array, properties_array, n):
        self.smiles_array = smiles_array
        self.len = np.shape(self.smiles_array)[0]
    ...
    def __getitem__(self, index):
        return tensorize(self.smiles_array[index, 0], self.n), torch.from_numpy(self.properties_array[index, :])
...
# and then executes it. Finally, outputs rand_walks for the entire batch in tensor form.
def tensorize(smiles, n):
    neighbor_dict = dict()
    mol = get_mol(smiles)
    ...
    return torch.from_numpy(rand_walk(fatoms, fbonds, neighbor_dict, n, mol)[0]).permute(2, 0, 1)
...
# Takes in a smiles, and outputs a random walk with information for testing. Does so in numpy array form.
def tensorize_test(smiles, n):
    neighbor_dict = dict()
    mol = get_mol(smiles)
    n_atoms = mol.GetNumAtoms()
    # Initialize neighbor set, as well as initialize and fill fatoms
...
def main():
    ...
    with open(fn) as f:
        ...
        for a in range(len(data)):
            rand_walk_comp, rand_walk_incomp = tensorize_test(data[a])
            print(sanity_check(rand_walk_comp, rand_walk_incomp, data[a]))
