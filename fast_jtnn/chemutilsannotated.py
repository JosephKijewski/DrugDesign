...
Sign in
Sign up
1718080wengong-jin/icml18-jtnn
 Code Issues 21 Pull requests 2 Projects 0 Security Insights
Join GitHub today
GitHub is home to over 40 million developers working together to host and review code, manage projects, and build software together.

icml18-jtnn/fast_jtnn/chemutils.py
 Wengong Jin fixing issues
1d29881 on Apr 4
430 lines (364 sloc)  16.4 KB
...
#Loads in a mol from smiles and kekulize
def get_mol(smiles):
...
# Takes in a 2d smiles and then changes it into what appears to be multiple 3d representations?
...
# Looks like this resets the molecule somehow.
def sanitize(mol):
    try:
...
# Creates new atom with all the same fields as some old atom
def copy_atom(atom):
...
# Copies a molecule. Bonds appear to be objects in the same sense that atoms are
...
# Takes a clique (set of atoms and their structure) and converts to a smiles
# string, then creates a new molecule that from this smiles string.
# the same as this clique
...
#Returns cliques and edges representing the tree decomposition of the input molecule
def tree_decomp(mol):
    n_atoms = mol.GetNumAtoms()
    if n_atoms == 1: #special case
        return [[0]], []

    #J Cliques appears to contain all bonds not in a ring represented as length two lists
    cliques = []
    for bond in mol.GetBonds():
        a1 = bond.GetBeginAtom().GetIdx()
        a2 = bond.GetEndAtom().GetIdx()
    ...
    #J this isn't entirely clear. I assume its getting the rings
    ssr = [list(x) for x in Chem.GetSymmSSSR(mol)]
    cliques.extend(ssr)

    #J This creates a list of lists, one list for each atom. Then, it appends the 
    #index of the cliques to each atom, so you get the list of cliques the atom
    #is in.
    ...
    #J Looks like this updates the nei list to include the condensed cliques
...
#J Checks that atoms have the same properties
def atom_equal(a1, a2):
    return a1.GetSymbol() == a2.GetSymbol() and a1.GetFormalCharge() == a2.GetFormalCharge()

#TRY TO UNDERSTAND THE LAST BIG FOR LOOP IN THE TREE DECOMP FUNCTION< THEN MOVE ON
